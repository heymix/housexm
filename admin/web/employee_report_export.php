...
where t.is_del=0 $sqlKey group by t.employee_id";
...
while($row = mysql_fetch_array($rs)) {
    ...
    ->setCellValue("C$i", $row['employee_tel'])
    ->setCellValue("D$i", $row['company_name'])
    ->setCellValue("E$i", $row['price_sum'])
    ->setCellValue("F$i", $row['price_status_is'])
    ->setCellValue("G$i", $row['price_status_not']);
}
